...
class SwinIR(nn.Module):
    ...
    def __init__(self,
        ...
        assert self.upscale==4, "only support scale 4"
    ...
    def forward_tail(self, x, feat, feat_size):
        feat = self.norm(feat)  # b seq_len c
        feat = self.patch_unembed(feat, feat_size)
    ...
    def forward(self, x):
        ...
        if not self.test_only: # training mode / eval mode
            ...
            feat_size = (feat.shape[2], feat.shape[3])
            ...
            for i, layer in enumerate(self.layers):
                feat = layer(feat, feat_size)
                if (i % self.exit_interval == (self.exit_interval-1)):
                    output = self.forward_tail(x, feat, feat_size)
                    decision = self.eedm(feat.permute(0,2,1))
            ...
            x = self.forward_head(x)
            feat = x * 1.0 # to assign, avoid just reference
            ...
            for i, layer in enumerate(self.layers):
                if len(pass_index) > 0:
                    feat[pass_index,...] = layer(feat[pass_index,...], feat_size)
                if (i % self.exit_interval == (self.exit_interval-1)):
                    decision = self.eedm(feat.permute(0,2,1))
