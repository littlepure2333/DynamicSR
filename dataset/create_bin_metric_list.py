...
if __name__ == '__main__':
    ...
    # save sorted list
    save_file = os.path.join(lr_dir,"total_x{}_descending.pt".format(scale))
    print("saving {}".format(save_file))
    with open(save_file, 'wb') as _f:
        pickle.dump(total_img_iy_ix, _f)

    save_file = os.path.join(lr_dir,"hard_x{}_descending.pt".format(scale))
    print("saving {}".format(save_file))
    with open(save_file, 'wb') as _f:
        pickle.dump(total_img_iy_ix[:total_len//3], _f)

    save_file = os.path.join(lr_dir,"midd_x{}_descending.pt".format(scale))
    print("saving {}".format(save_file))
    with open(save_file, 'wb') as _f:
        pickle.dump(total_img_iy_ix[total_len//3:total_len*2//3], _f)

    save_file = os.path.join(lr_dir,"easy_x{}_descending.pt".format(scale))
    print("saving {}".format(save_file))
    with open(save_file, 'wb') as _f:
