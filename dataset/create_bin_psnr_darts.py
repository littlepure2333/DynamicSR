...
def gen_psnr_darts(psnr_map_file, n_samples = 1000):
    ...
    # print(patch_size_lr)
    # print(darts_uvp_list_lr[:,0].max())
    # print(darts_uvp_list_lr[:,1].max())

    # get importance map
    # imp_map = 1 / psnr_map
    # imp_map = imp_map / imp_map.max()

    # prune patches
    # pad = patch_size_lr // 2
    # pruned = np.maximum(0, prunePatches(img_dim_lr, darts_list + pad, patch_size_lr, imp_map) - pad)
    # selection = psnr_map * 0.1
    # for i in range(pruned.shape[0]):
    #     x, y = pruned[i, 0], pruned[i, 1]
    #     selection[y:y + patch_size_lr, x:x + patch_size_lr, :] = psnr_map[y:y + patch_size_lr, x:x + patch_size_lr, :]
    
    # sort index
    sort_index = np.argsort(darts_uvp_list_lr[:,-1])
    darts_uvp_list_lr = darts_uvp_list_lr[sort_index]

    psnr_file = psnr_map_file.replace("_psnr_map_up.pt","_psnr_darts_1000.pt")
...
if __name__ == '__main__':
    psnr_map_dir = "/data/shizun/dataset/DIV2K/bin/DIV2K_train_LR_bicubic/X2/"
