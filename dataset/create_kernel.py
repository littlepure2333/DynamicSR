...
scale = 2
scale_factor = np.array([scale, scale])  # choose scale-factor
avg_sf = np.mean(scale_factor)  # this is calculated so that min_var and max_var will be more intutitive
min_var = 0.175 * avg_sf  # variance of the gaussian kernel will be sampled between min_var and max_var
max_var = 2.5 * avg_sf
k_size = np.array([21, 21])  # size of the kernel, should have room for the gaussian
noise_level = 0.4  # this option allows deviation from just a gaussian, by adding multiplicative noise
images_path = './DIV2K/HR/*.png'
output_path = './DIV2K/LR_RK/X%d' % scale
...
for i, path in enumerate(glob.glob(images_path)):
    ...
    # plt.show()
    lr = np.clip(lr, 0, 1)
    savemat(os.path.join(output_path, '%s.mat' % basename), {'ker': kernel})
    plt.imsave(os.path.join(output_path, '%s.png' % basename), lr, vmin=0, vmax=1)
