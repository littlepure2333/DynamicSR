...
class Trainer():
    ...
    def train(self):
        ...
        self.ckp.write_log(
            '[Epoch {}]\t[Exit {}/{}]\tLearning rate: {:.2e}'.format(epoch, self.exit_index, len(self.exit_list), Decimal(lr))
        )
        self.loss.start_log()
        self.model.train()
    ...
    def test(self):
        ...
        for idx_data, d in enumerate(self.loader_test):
            for idx_scale, scale in enumerate(self.scale):
                ...
                epoch = self.optimizer.get_last_epoch() + 1 + self.prev_epoch
                self.current_exit_best = self.ckp.log[self.prev_epoch:].max(0)
                self.ckp.write_log(
    ...
    def terminate(self):
        if self.args.test_only:
            ...
            epoch = self.optimizer.get_last_epoch() + self.prev_epoch
            if self.best and ((epoch - int(self.best[1])) > self.args.conv_thre): # This exit has converged, then optimize next exit
                self.prev_epoch = epoch
                self.exit_index += 1
                self.ckp.write_log(
                    'Exit {} has converged, now optimize at exit {}'.format(self.exit_index-1, self.exit_index), refresh=True
                )
                if self.exit_index >= len(self.exit_list): return True # every exit has converged
                self.model.apply(lambda m: setattr(m, 'exit_index', self.exit_index))
                if self.args.freeze:
                    for m in self.model.model.head.parameters():
                        m.requires_grad = False
                    for m in self.model.model.body[:self.exit_list[self.exit_index]].parameters():
                        m.requires_grad = False
                    if not self.args.shared_tail:
                        for m in self.model.model.tail[:self.exit_index].parameters():
                            m.requires_grad = False
                self.optimizer = self.optimizer = utility.make_optimizer(self.args, self.model)
                for name, param in self.model.named_parameters():
                    self.ckp.write_log(name, param.size(), param.requires_grad)
